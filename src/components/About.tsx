...
const About = () => {
  return (
    <section
      id="sobre-mi"
      className="flex flex-col items-center justify-center px-8 md:px-16 py-12 md:py-24 w-full max-w-[1440px] mx-auto"
    >
      <div className="max-w-4xl text-center flex flex-col gap-8 mb-12 md:mb-20">
        ...
        <div className="text-primary text-md md:text-xl font-regular flex flex-col gap-6 px-4 leading-tight">
          <p>
            Entrenadora personal con más de +5 años de experiencia, certificada
            por la FVFC (Federación Venezolana de Fisicoculturismo y Fitness)
            especializada en entrenamientos de alta intensidad y desarrollo
            muscular.
          </p>
          <p>
            Como atleta de alto rendimiento, mi vida se basa en buscar nuevos
            retos cada día, y mi objetivo es transmitir esa misma energía para
            crear una comunidad activa que supere sus propios límites
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:hidden">
        <MobileSlider interval={3000}>
          {slides.map((slide) => (
            <div
              key={slide.id}
              className="relative w-full aspect-[3/4] rounded-3xl overflow-hidden group shadow-xl"
            >
              <Image
                src={slide.img}
                alt={slide.title}
              ...
              <div className="absolute inset-0 bg-primary/50 group-hover:bg-primary/0 transition-colors duration-300 flex items-center justify-center">
                <h3 className="text-white text-2xl md:text-3xl font-bold tracking-wide drop-shadow-md uppercase">
                  {slide.title}
                </h3>
              </div>
            </div>
